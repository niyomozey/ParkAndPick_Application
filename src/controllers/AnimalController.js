import {Animal, Slaughterhouse} from '../../sequelize/models'
...
class AnimalController{
  ...
  async createAnimal(req, res) {
    ...
    const  { animalId, weight, type, SlaughterhouseId  } = req.body
    Animal.create({ animalId, weight, type, SlaughterhouseId })
      .then(data => {
        res.status(201).json({data, message: 'Animal created successfully!'});
      })
      .catch(err => {
        res.status(500).json({
          message:
...
async findAllAnimals(req, res) {
    Animal.findAll()
      .then(data => {
        res.status(200).json({message: 'List of all Animals',data});
      })
      .catch(err => {
        res.status(500).json({
          message:
...
async countAnimalsBySlaughterHouseName(req, res)
{
  ...
  Slaughterhouse.findOne({where : {name: name}}).then(data =>{
    if(data)
    {
      const {id} = data
      Animal.findAll({where: {SlaughterhouseId: id}}).then(animal =>{
        let count = 0;
        animal.map( items => {
          return count ++;
        })
        if(count > 0)
          return res.status(200).json({message: `Found ${count} animal`, numberOfAnimal: count, animal})
        else
      ...
      res.status(404).json({message: 'Animal not found!! Try with another slaughter house.'})
    }
  }).catch(error =>{
    res.status(404).json({
...
async findAnimalBySlaughterhouseName(req, res){
  ...
  Slaughterhouse.findOne({where : {name: name}}).then(data =>{
    ...
    {
      const {id} = data
      Animal.findOne({where: {SlaughterhouseId: id}}).then(animal =>{
        if(animal)
          return res.status(200).json({message: 'Animal found successfuly', data: animal})
        else
          throw new Error('Animal not found')
      }).catch(err => {
        res.status(404).json({ message: err.message})
